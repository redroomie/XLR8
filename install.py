...
def check_lang():
    try:
        if path.isfile('XLR8conf/langconf.txt') == True:
            with open('XLR8conf/langconf.txt') as file_lang:
...
def choose_lang(language=English):
    try:
        with open('XLR8conf/langconf.txt', mode="w") as file_lang:            
...
def uninstall():
    try:
        ...
        if choice[0].upper() == "Y":
            ...
            system('rm -rf /usr/bin/XLR8conf')
            system('rm /usr/bin/XLR8')
            print(language.uninstalled)
            
        else:
            print(language.XLR8_tip.format('XLR8') + color.END)
...
if path.isfile('/usr/bin/XLR8') == True:
    print(language.already_installed.format('XLR8'))
    uninstall()
...
def install_package(package):
    try:
        if path.isfile('/usr/bin/'+package) == True:
            ...
            if path.isfile('/usr/bin/upgradepkg') == True:
                print(language.XLR8_tip.format('python3 XLR8.py'))
                print(language.video_tutorials)
                exit()
...
def pyinstaller():
    try:
        ...
        print(language.installing.format('XLR8'))
        system('pyinstaller --onefile XLR8.py')
        system('cp -r dist/XLR8 /usr/bin && cp -r XLR8conf /usr/bin')
        system('chmod +x /usr/bin/XLR8')
