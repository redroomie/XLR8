...
try:
    from XLR8conf.theme import *
except ModuleNotFoundError:
    print("XLR8 is lacking its needed files. Reinstall XLR8 from Github pls")
    exit()
...
def the_argparse(language=English):
        parser = argparse.ArgumentParser(usage="XLR8 [-h] -s|-x|-id|-r|-m|-c|-l|--list")
...
if (path.isfile('/usr/bin/upgradepkg') == True) or (path.isfile('/usr/bin/XLR8conf/langconf.txt') == False):
    LANGCONF = 'XLR8conf/langconf.txt'
else:
    LANGCONF = '/usr/bin/XLR8conf/langconf.txt'
...
update_commands = """cd ~ && rm -rf XLR8
git clone https://github.com/gitkern3l/XLR8
cd XLR8
...
def check_update():
    try:
        ...
        version = getoutput('curl -s --max-time 60 https://raw.githubusercontent.com/gitkern3l/XLR8/master/XLR8conf/Version')
        sleep(SLEEP_TIME)
        print(language.done)
...
def start_connecting(id=None):
    try:
        ...
        if (path.isfile(Torrc)) and (torrconfig == open(Torrc).read()):
            print(language.already_configured.format('XLR8 Torrc'))
            
        else:
            print(language.configuring.format('XLR8 Torrc'), end='', flush=True)
